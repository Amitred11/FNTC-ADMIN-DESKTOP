document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderTable = () => {
        ...
        const filteredData = state.jobOrders.filter(job => {
            ...
            const searchableText = [customerName, assignedTech, job._id, shortId, job.type, job.status].join(' ').toLowerCase();
        ...
        filteredData.forEach(job => {
            ...
            row.innerHTML = `<td>${job._id.slice(-6).toUpperCase()}</td><td>${customerName}</td><td>${job.type}</td><td><span class="status-badge ${statusClass}">${job.status}</span></td><td>${assignedTech}</td><td class="actions-cell"><button class="btn sm view-detail">View</button><button class="btn sm outline assign-tech" ${isAssignable ? '' : 'disabled'}>Assign</button></td>`;
    ...
    const fetchAndRenderArchives = async () => {
        ...
        try {
            ...
            if (response.ok && response.data.length > 0) {
                listContainer.innerHTML = response.data.map(job => `<li class="archive-list-item"><span><strong>${job._id.slice(-6).toUpperCase()}</strong> — ${job.type} — Archived on ${new Date(job.archivedAt).toLocaleDateString()}</span><button class="btn sm delete-archive-btn" data-id="${job._id}">Delete</button></li>`).join('');
            } else {
        ...
        if (e.target.classList.contains('delete-archive-btn')) {
            ...
            try {
                await AppAlert.confirmOnDialog({
                    type: 'danger',
                    title: 'Delete Job?',
                    message: `Are you sure you want to permanently delete archived job ${jobId.slice(-6).toUpperCase()}? This cannot be undone.`,
                ...
                if (error) { // An actual error occurred
                     AppAlert.notify({ type: 'error', title: 'Error', message: error.message });
                }
                // If no error, it means the user cancelled, so we do nothing.
            }
        }
    });
    ...
    if (createOrderForm) {
        ...
        createOrderForm.addEventListener('submit', async (e) => {
            ...
            const body = {
                ...
                autoAssign: formData.get('assignment') === 'auto',
                agentId: formData.get('assignment') === 'manual' ? formData.get('agentId') : null
            };
